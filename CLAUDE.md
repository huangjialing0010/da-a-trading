...
- **数据熔断**：日更按每只趋势持仓分别核对K线日期；任一持仓K线缺失，或最旧持仓日期早于基准缓存日期（`data/market/benchmark_000300.csv` 最后一行），当日趋势仓买入和卖出均不执行、不记录净值/表现，并逐只列出异常代码。基准日期本身缺失只告警、不阻断交易（基准只影响超额对比，不影响买卖信号）

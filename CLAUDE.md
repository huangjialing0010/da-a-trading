...
- 趋势反转策略（利润趋势改善+质量过滤+MA200退出）：+79.0% vs 基准+65.9%，超额+13%，每年有交易
- 趋势反转严格版胜率44%、盈亏比2:1、最大回撤-46%
...
## 趋势反转策略参数（纸上测试）
- 入场：利润YoY连续2个同类型报告期改善（≥2pp），ROE>6%，负债<60%，不追52周高点（<90%分位）
- 退出：同深价（硬止损/MA200/移动止盈/到期）
- 仓位：最多5只，单只20%资金
- 质量过滤：跳过改善>500pp（基数效应）、改善<5pp（噪音）、利润YoY<-20%（仍在恶化）

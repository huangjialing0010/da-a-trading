...
## 变更与交接记录
当前状态（2026-08-04）：基准价已切换新浪为主/东财兜底；趋势仓数据熔断、终审样本不足顺延、交易成本模型已生效；市场判断台账每周五回填；周报/月报已含双策略组合总览（深价100万+趋势200万按初始资金权重合成净值、回撤、波动对比）；深价仓定位写回左侧交易仓；ERP 动态仓位上限+行业集中度已接入代码（自动加仓、恐慌买入、手动买入、信号生成均校验），解除旧版一刀切冻结。详细历史见 `docs/CHANGES.md`。
...
- **样本不足顺延**：若 2027-01-01 完整买卖回合 < 30，判定样本不足，不判通过/作废，顺延 3 个月再评，并附注当期市场状态（沪深300区间涨跌 + ERP分位）；完整买卖回合 ≥30 时严格按终审三条件执行，不因市场环境顺延
- **有效验证切点**：账户历史继续保留，但候选母集修复后的实盘准入样本从 2026-08-07 独立计算。只有该日及之后新开仓并完成清仓的回合计入有效30回合；切点前开仓的持仓未来卖出只计历史全量。日报与趋势周报同时显示有效收益、基准、超额、最大回撤和四项准入条件
- **终审结果**：
